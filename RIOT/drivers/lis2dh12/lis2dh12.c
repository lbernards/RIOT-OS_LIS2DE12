                              ...
                              uint8_t click, uint8_t line)
{
    ...
    uint8_t odr   = _read(dev, REG_CTRL_REG1) >> 4;
    uint8_t scale = (_read(dev, REG_CTRL_REG4) >> 4) & 0x3;//0x3;
...
void lis2dh12_read_click_event(const lis2dh12_t *dev, uint8_t *buffer, uint8_t registre)
{
    _acquire(dev);

    *buffer = _read(dev, registre);

    _release(dev);
}

void lis2dh12_init_int(const lis2dh12_t *dev){ // Activate all the interruptions events and click
    _acquire(dev);
    _write(dev, 0x22, 0xE0);
    _release(dev);
}

void lis2dh12_init_ths1(const lis2dh12_t *dev){ //configurate threshold of event 1 to +2G 0x7D
    _acquire(dev);
    _write(dev, 0x32, 0x7D);
    _release(dev);
}

void lis2dh12_init_ths2(const lis2dh12_t *dev){ //configurate threshold of event 2 to +2G
    _acquire(dev);
    _write(dev, 0x36, 0x7D);
    _release(dev);
}

void lis2dh12_init_reg5(const lis2dh12_t *dev){ //activate interrupt request latched 1 and 2
    _acquire(dev);
    _write_or(dev, 0x24, 0x10);
    _release(dev);
}
...
int lis2dh12_wait_event(const lis2dh12_t *dev, uint8_t line, bool stale_events)
{
    ...
    /* If pin is valid interruption works with assync interruption */
    ...
    /*If there is no interruption pin, set the interruption verification time according to application*/
    else
    {
        /*Each 5 seconds we verify if an interruption was made*/
        xtimer_sleep(5); 
    }
