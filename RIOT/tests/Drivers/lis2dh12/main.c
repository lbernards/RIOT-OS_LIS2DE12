...
void lis2dh12_test_init(void) {
    ...
    /* change LIS2DH12 settings so it will also has compatibility with LIS2DE12*/
    lis2dh12_set_resolution(&dev, LIS2DH12_POWER_LOW);
    lis2dh12_set_datarate(&dev, LIS2DH12_RATE_VERYHIGH);
    lis2dh12_set_scale(&dev, LIS2DH12_SCALE_2G);

    /*Interruption configuration*/
    lis2dh12_init_reg5(&dev);
    lis2dh12_init_int(&dev);
    lis2dh12_init_ths1(&dev);
    lis2dh12_init_ths2(&dev);
...
/**
 * @brief This function is made to verify if there is any interruption event, or click event
*/
void* lis2dh12_test_process(void* arg) {
    ...
    while (1) {
        ...
        /* wait for interrupt */
        int int1_src = lis2dh12_wait_event(&dev, LIS2DH12_INT1, false); //int_src = click, function 1 or funciton 2 

        if (int1_src <= 0) {
            printf("No interruption: %d\n", int1_src); 
            continue;
        }

        /*Verifying wich kind of interruption was made*/
...
/**
 * @brief accelaration reading function
*/
...
/**
 * @brief Function to read any register of lis2d*12
*/
static int shell_is2dh12_read_register(int argc, char **argv){

   uint8_t buffer; //buffer to read register
   uint8_t b_values[8]; //values of each bit
   uint8_t register_to_read = 0;

   char *endptr;

    if (argc > 1) {
        register_to_read = strtol(argv[1], &endptr, 16);
    }
    else{
        printf("Usage: read_int register(Hexa) \n");
    }

    /*verifying if conversion string -> hexa works*/ 
    if (*endptr != '\0') {
        printf("Error. Can't read hexa value.\n");
        return 1;
    }

   /*reading register and keeping it in buffer*/
   lis2dh12_read_click_event(&dev, &buffer, register_to_read); 

    /*Loop extracting each bit of buffer*/
   for (uint8_t i = 0; i < 8; i++)
   {
        uint8_t mask=1 << i;
        b_values[i] = (mask & buffer) >> i; 
   }

    printf(" bit_7: %d bit_6: %d  bit_5: %d  bit_4: %d  bit_3: %d  bit_2: %d  bit_1: %d  bit_0: %d \n", b_values[7],b_values[6], b_values[5], b_values[4], b_values[3], b_values[2], b_values[1], b_values[0]);
...
static const shell_command_t shell_commands[] = {
    ...
    { "read_int", "Read any register from accelerometer", shell_is2dh12_read_register },
    { NULL, NULL, NULL },
};
...
/*Thread for checking interruption*/
#ifdef MODULE_LIS2DH12_INT
